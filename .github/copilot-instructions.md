# Copilot Instructions for Markdown Region Buddy

## Project Overview

Markdown Region Buddy is a VS Code extension that helps authors manage markdown regions — monikers, zone pivots, and tabs — with folding, hover previews, focus commands, and optional background decorations. It activates on `markdown` files and targets VS Code `^1.85.0`.
...
- Configuration is read via `vscode.workspace.getConfiguration('markdownRegionBuddy')`.

### Extension Manifest (`package.json`)

- All commands use the `markdownRegionBuddy.` prefix
- Command titles follow the pattern: `Markdown Region Buddy: <Action Description>`
- Context menus are gated with `"when": "editorLangId == markdown"`
- Keybindings use `"when": "editorTextFocus && editorLangId == markdown"`
- The extension uses a `markdownRegionBuddy.context` submenu in the editor context menu
...
1. Add the command ID and title to `contributes.commands` in `package.json`, using the `markdownRegionBuddy.<camelCaseAction>` naming pattern and the `Markdown Region Buddy: <Title>` display format.
...
5. If the command should appear in the context menu, add it to the `markdownRegionBuddy.context` submenu with `"when": "editorLangId == markdown"`.
...
1. Add the property under `contributes.configuration.properties` in `package.json` with the `markdownRegionBuddy.` prefix.
2. Read it in the appropriate module via `vscode.workspace.getConfiguration('markdownRegionBuddy')`.
...
- **`vscode-ext-commands`** — Guidelines for contributing commands: naming (`markdownRegionBuddy.<action>`), categories, visibility (Command Palette vs Side Bar), icons, `enablement`, `when` clauses, and ordering.
